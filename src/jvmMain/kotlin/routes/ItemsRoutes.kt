...
fun Route.itemsRouting() {
    route("/items") {
        get {
            ...
            if (!call.receiveParameters().isEmpty()) {
                ...
                if (call.receiveParameters()["isbn"] != null)
                    filterConditions.add("isbn = " + call.receiveParameters()["isbn"])
                if (call.receiveParameters()["rlbc"] != null)
                    filterConditions.add("rlbc = " + call.receiveParameters()["rlbc"])
                if (call.receiveParameters()["title"] != null)
                    filterConditions.add("title = " + call.receiveParameters()["title"])
                if (call.receiveParameters()["authors"] != null)
                    filterConditions.add("authors = " + call.receiveParameters()["authors"])
                if (call.receiveParameters()["type"] != null)
                    filterConditions.add("type = " + call.receiveParameters()["type"])
                if (call.receiveParameters()["details"] != null)
                    filterConditions.add("details = " + call.receiveParameters()["details"])
        ...
        post("/insert") {
            val isbn = call.receiveParameters()["isbn"] ?: return@post call.respondText(
                "Missing or malformed isbn",
                status = HttpStatusCode.BadRequest
            )
            val rlbc = call.receiveParameters()["rlbc"] ?: return@post call.respondText(
                "Missing or malformed rlbc",
                status = HttpStatusCode.BadRequest
            )
            val title = call.receiveParameters()["title"] ?: return@post call.respondText(
                "Missing or malformed title",
                status = HttpStatusCode.BadRequest
            )
            val authors = call.receiveParameters()["authors"] ?: return@post call.respondText(
                "Missing or malformed authors",
                status = HttpStatusCode.BadRequest
            )
            val type = call.receiveParameters()["type"] ?: return@post call.respondText(
                "Missing or malformed type",
                status = HttpStatusCode.BadRequest
            )
            val details = call.receiveParameters()["details"] ?: return@post call.respondText(
